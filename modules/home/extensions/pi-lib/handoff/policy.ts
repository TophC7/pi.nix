...
export async function deferToAgentEnd(pi: ExtensionAPI, fn: (ctx: ExtensionContext) => Promise<void> | void): Promise<void> {
...
async function chooseConfirmation<C extends HandoffContext>(options: HandoffOptions<C>): Promise<"run" | "edit" | "cancel"> {
...
function markAwaitingConfirmation<C extends HandoffContext>(options: HandoffOptions<C>): void {
	try {
		if (workflowController.status === "active") workflowController.markAwaitingConfirmation(options.pi, options.ctx, options.reason ?? `${options.label}: awaiting_confirmation`);
	} catch (error) {
...
function markContinuationQueued<C extends HandoffContext>(options: HandoffOptions<C>): void {
	try {
		if (workflowController.status === "active" || workflowController.status === "awaiting_confirmation") {
			workflowController.markContinuationQueued(options.pi, options.ctx, options.reason ?? `${options.label}: continuation_queued`);
		}
	} catch (error) {
...
function markManualPending<C extends HandoffContext>(options: HandoffOptions<C>): void {
	try {
		if (workflowController.status === "active" || workflowController.status === "awaiting_confirmation") {
			workflowController.markManualPending(options.pi, options.ctx, options.reason ?? `${options.label}: manual_pending`);
		}
	} catch (error) {
