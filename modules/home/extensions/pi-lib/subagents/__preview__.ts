...
function makeTool(
  name: string,
  status: SubagentToolSnapshot['status'],
  ...
  const slot = makeSlot({
    ...
    tools: [
      makeTool(
        ...
        'completed',
        {
          path: '/repo/Nix/pi.nix/modules/home/extensions/pi-lib/subagents/types.ts'
        },
        4000
      ),
      ...
      makeTool(
        ...
        'completed',
        {
          path: '/repo/Nix/pi.nix/modules/home/extensions/pi-lib/subagents/engine.ts'
        },
        1900
      ),
...
function parallelInFlight(): SubagentRunState {
  const slots = [
    ...
    makeSlot({
      ...
      tools: [makeTool('bash', 'running', { command: 'tokei modules/home/extensions/pi-lib' }, 600)]
    })
  ]
  return baseState({
    id: 'run-3',
    ...
    makeSlot({
      ...
      tools: [
        makeTool(
          ...
          'running',
          {
            path: '/repo/Nix/pi.nix/modules/home/extensions/pi-lib/subagents/render.ts'
          },
          1400
        )
      ]
...
function parallelMixedFailure(): SubagentRunResult {
  const slots = [
    ...
    makeSlot({
      ...
      status: 'failed',
      toolCount: 5,
      error: 'Pi runtime aborted: model returned malformed tool call',
      usage: { inputTokens: 1500, outputTokens: 600, turns: 3 },
      duration: {
...
function baseResult(input: Pick<SubagentRunResult, 'id' | 'mode' | 'label' | 'slots' | 'status'>): SubagentRunResult {
  ...
  return {
    ...
    events: [],
    finalText: input.slots
      .map((slot) => slot.output.finalOutput ?? '')
      .filter(Boolean)
      .join('\n\n'),
    rerun: {
      decision: 'not-needed',
