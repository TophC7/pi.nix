...
export function capRecentOutput(previous: string, delta: string): CappedText {
	...
	return {
		text: byBytes.text,
		truncation: combineTruncation(byLines.truncation, byBytes.truncation, "Recent output capped to last 20 lines / 16 KiB."),
	};
}
...
function tailBytes(text: string, maxBytes: number): CappedText {
	...
	const kept = buffer.subarray(Math.max(0, buffer.length - maxBytes)).toString("utf8").replace(/^�+/, "");
	return {
		text: kept,
		truncation: {
			truncated: true,
