...
export function renderSubagentRun(run: Renderable, options: RenderOptions, theme: Theme): Component {
	...
	return options.expanded
		? renderMultiExpanded(run, theme, width)
		: renderMultiCompact(run, theme, width);
}
...
function renderSingleCompact(run: Renderable, slot: SubagentSlotResult, theme: Theme, width: number): Component {
	...
	if (slot.status === "running") {
		...
		c.addChild(new Text(truncLine(theme.fg("error", `  ⎿  ${truncate(singleLine(slot.error), width - 6)}`), width), 0, 0));
	}
...
function renderSingleExpanded(run: Renderable, slot: SubagentSlotResult, theme: Theme, width: number): Component {
	...
	const stats = slotStats(theme, slot);
	c.addChild(new Text(`${verb} ${theme.fg("toolTitle", themeBold(theme, slot.agent))}${stats ? ` ${theme.fg("dim", "·")} ${stats}` : ""}`, 0, 0));
	...
	if (isRunning) {
		...
		if (liveTool) c.addChild(new Text(theme.fg("warning", `> ${formatToolCall(liveTool.name, liveTool.args, true)}`), 0, 0));
...
function renderMultiCompact(run: Renderable, theme: Theme, width: number): Component {
	...
	for (const slot of run.slots) {
		...
		if (slot.status === "running") {
			...
			c.addChild(new Text(truncLine(theme.fg("error", `    ⎿  ${truncate(singleLine(slot.error), width - 8)}`), width), 0, 0));
		}
	}
	if (run.slots.some((slot) => slot.status === "running")) {
		c.addChild(new Text(theme.fg("accent", "  Press Ctrl+O for live detail"), 0, 0));
...
function buildRunHeader(run: Renderable, theme: Theme): string {
	...
	statusParts.push(usage.inputTokens + usage.outputTokens > 0 ? formatTokenStat(usage.inputTokens + usage.outputTokens) : undefined);
...
function currentActivity(slot: SubagentSlotResult, maxWidth: number): string {
	if (slot.currentTool) {
		const tool = slot.tools.find((entry) => entry.name === slot.currentTool && entry.status === "running") ?? slot.tools.at(-1);
		if (tool) {
