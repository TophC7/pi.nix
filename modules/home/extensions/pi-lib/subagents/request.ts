...
export type RequestNormalization = { ok: true; request: NormalizedRequest } | { ok: false; error: string }
...
export function normalizeSubagentRequest(
  ...
  if (Array.isArray(params.tasks)) {
    ...
    if (totalSlots === 0)
      return {
        ok: false,
        error: "Subagent request 'tasks' produced no runnable slots."
      }
    if (totalSlots > SUBAGENT_MAX_PARALLEL_SLOTS) {
      return {
        ok: false,
        error: `Subagent parallel slots ${totalSlots} exceed cap ${SUBAGENT_MAX_PARALLEL_SLOTS}. Reduce 'count' or split.`
      }
    }
    return {
      ok: true,
  ...
  return {
    ok: false,
    error: 'Subagent request requires { agent, task }, { tasks: [...] }, or { chain: [...] }.'
  }
}
...
function parseChain(rawChain: unknown[]): { ok: true; steps: NormalizedChainStep[] } | { ok: false; error: string } {
  ...
  if (rawChain.length > SUBAGENT_MAX_CHAIN_STEPS) {
    return {
      ok: false,
      error: `Subagent chain exceeds cap ${SUBAGENT_MAX_CHAIN_STEPS}.`
    }
  }
  ...
  for (let index = 0; index < rawChain.length; index++) {
    ...
    return {
      ok: false,
      error: `chain[${index}] needs 'agent'+'task' or a 'parallel' array.`
    }
  }
...
function parseTaskEntry(
  value: unknown,
  location: string
): { ok: true; task: NormalizedTask } | { ok: false; error: string } {
  ...
  if (typeof data.agent !== 'string' || !data.agent.trim())
    return { ok: false, error: `${location} requires non-empty 'agent'.` }
  if (typeof data.task !== 'string' || !data.task.trim())
    return { ok: false, error: `${location} requires non-empty 'task'.` }
...
function parseCount(value: unknown, location: string): { ok: true; value: number } | { ok: false; error: string } {
  ...
  if (typeof value !== 'number' || !Number.isFinite(value))
    return { ok: false, error: `${location}.count must be a finite number.` }
  const integer = Math.floor(value)
  if (integer < 1) return { ok: false, error: `${location}.count must be ≥ 1.` }
  if (integer > SUBAGENT_MAX_TASK_COUNT) {
    return {
      ok: false,
      error: `${location}.count ${integer} exceeds cap ${SUBAGENT_MAX_TASK_COUNT}.`
    }
  }
...
export function parseSystemPromptMode(value: unknown): SubagentSystemPromptMode | undefined {
  if (typeof value !== 'string') return undefined
  return (SUBAGENT_SYSTEM_PROMPT_MODES as readonly string[]).includes(value)
    ? (value as SubagentSystemPromptMode)
    : undefined
}
