...
import { SUBAGENT_ANIMATION_MS, spinnerFrameForTick } from './render-helpers.ts'
import { buildRunRequest, normalizeSubagentRequest } from './request.ts'
import {
  combineSubagentUsage,
  mapEngineEventToLiveLog,
...
function handleRunUpdate(
  ...
  if (update.type === 'run-start') {
    ...
    publishSubagentStatus(
      statusKey,
      runningSlot?.currentTool ? `${label}: ${runningSlot.currentTool}` : `${label}: running`,
      true
    )
...
async function recoverParallelFailure(
  ...
  const failedNames = result.slots
    .filter((slot) => failedIds.includes(slot.id))
    .map((slot) => slot.agent)
    .join(', ')
  const shouldRerun = ctx.hasUI
    ? await ctx.ui.confirm(
        `${label}: rerun failed subagent(s)?`,
...
function mergeRerunResult(
  original: SubagentRunResult,
  rerun: SubagentRunResult,
  failedIds: string[]
): SubagentRunResult {
  const replacements = new Map(rerun.slots.map((slot) => [slot.id, slot]))
  original.slots = original.slots.map((slot) =>
    failedIds.includes(slot.id) ? (replacements.get(slot.id) ?? slot) : slot
  )
...
function toSubagentResponse(result: SubagentRunResult): SubagentResponse {
  ...
  const partialNote =
    result.status === 'partial' && failedAgents.length > 0
