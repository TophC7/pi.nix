...
import { type AgentScope, buildRunRequest, isAgentScope, normalizeSubagentRequest } from './request.ts'
...
export function registerSubagentTool(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    label: 'Subagent',
    description:
      'Delegate tasks to local in-process subagents. Supports list/get, single agent, parallel tasks, and chain.',
    ...
    async execute(
      ...
      if (params.action === 'get' && typeof params.agent === 'string') {
        return {
          content: [{ type: 'text', text: formatAgentDetails(agents, params.agent) }]
        }
      }
      const normalization = normalizeSubagentRequest(params, {
        defaultLabel:
          Array.isArray(params.tasks) || Array.isArray(params.chain)
            ? 'subagents'
      ...
      const request = buildRunRequest(normalization.request, {
        cwd,
        parentSignal: signal
      })
      ...
      return {
        content: [
          {
            type: 'text',
            text: result.finalText || result.error || 'No subagent output.'
          }
        ],
        details: { result },
...
export function assertNoSubagentToolOwner(pi: ExtensionAPI): void {
  ...
  if (owner) {
    throw new Error(
      `Cannot register local subagent tool: tool name "subagent" is already registered${owner.description ? ` (${owner.description})` : ''}.`
    )
  }
}
...
function formatAgentList(agents: ReturnType<typeof discoverAgents>): string {
  ...
  return agents
    .map((agent) =>
      `${agent.runtimeName} [${agent.source}]${agent.model ? ` · ${agent.model}` : ''}: ${agent.description ?? ''}`.trim()
    )
    .join('\n')
}
