...
function sortStatus(a: UiStatusEntry, b: UiStatusEntry): number {
	return PRIORITY_ORDER[a.ordering.priority] - PRIORITY_ORDER[b.ordering.priority]
...
function makeSnapshot(statuses: Iterable<UiStatusEntry>, widgets: Iterable<UiWidgetEntry>, version: number): UiSnapshot {
	return {
...
class SharedUiStatusStore implements UiStatusStoreWriter {
	...
	publishStatus(entry: UiStatusEntry): UiPublicationHandle {
		...
		if (existing && sameStatusRenderData(existing, entry)) {
			this.statuses.set(entry.id, { ...entry, lifecycle: { ...entry.lifecycle, createdAt: existing.lifecycle.createdAt } });
	...
	publishWidget(entry: UiWidgetEntry): UiPublicationHandle {
		...
		if (existing && sameWidgetRenderData(existing, entry)) {
			this.widgets.set(entry.id, { ...entry, lifecycle: { ...entry.lifecycle, createdAt: existing.lifecycle.createdAt } });
