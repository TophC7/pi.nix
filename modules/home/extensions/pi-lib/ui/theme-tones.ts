...
function normalizeThinking(level: string | undefined | null): ThinkingLevelName {
	if (level === "minimal" || level === "low" || level === "medium" || level === "high" || level === "xhigh") return level;
	return "off";
}
...
function makeThinkingToneSgr(base: RgbColor, background: RgbColor): Record<ThinkingLevelName, string> {
	...
	const quiet = mix(background, base, dark ? 0.36 : 0.30);
	const low = mix(background, base, dark ? 0.56 : 0.50);
...
function xterm256ToRgb(index: number): RgbColor {
	...
	if (safe < 232) {
		...
		return { r: level(Math.floor(cube / 36)), g: level(Math.floor((cube % 36) / 6)), b: level(cube % 6) };
	}
