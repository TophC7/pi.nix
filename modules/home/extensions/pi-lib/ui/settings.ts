...
export function showSettingsPane(
	...
	options: SettingsPaneOptions,
) {
	return openDialog(
		ctx,
		({ theme, close }) => new SettingsDialogBody(theme, options, close),
		{
			width: "96%",
			maxHeight: "90%",
			minWidth: 60,
...
class SettingsDialogBody implements DialogContent {
	...
	render(width: number): string[] {
		...
		const header = renderDialogHeader({ title: this.options.title, theme: this.theme, width: safeWidth, searchInput: this.searchInput });
		const footer = renderDialogFooter({ theme: this.theme, width: safeWidth, keys: this.options.footer ?? FOOTER_KEYS, status: this.statusLine(items) });
		...
		const lines: string[] = [
			header,
			renderDialogDivider({ theme: this.theme, width: safeWidth, splitAt: widths.list }),
		];
		for (let index = 0; index < bodyRows; index += 1) {
			lines.push(`${listLines[index] ?? " ".repeat(widths.list)}${vertical}${inspectorLines[index] ?? " ".repeat(widths.inspector)}`);
		}
		lines.push(renderDialogDivider({ theme: this.theme, width: safeWidth, splitAt: widths.list }));
		lines.push(footer);
		return lines;
	}
	...
	private listLines(items: readonly SettingsPaneItem[], width: number, rowCount: number): string[] {
		...
		return padRows(visible.map((item) => this.settingRow(item, safeWidth, labelWidth, valueWidth)), rowCount, safeWidth);
	}
	...
	private settingRow(item: SettingsPaneItem, width: number, labelWidth: number, valueWidth: number): string {
		...
		const value = selected ? this.theme.fg("accent", fit(item.currentValue, valueWidth)) : this.theme.fg("muted", fit(item.currentValue, valueWidth));
	...
	private inspectorLines(item: SettingsPaneItem | undefined, width: number, rowCount: number): string[] {
		...
		top.push(...this.wrapped(item.detailSummary ?? ["Cycle available values with Space, Enter, or arrow keys."], safeWidth, "muted"));
		...
		const bottom: string[] = [];
		if (item.detailPath) bottom.push(this.theme.fg("dim", fit(" Path:", safeWidth)), this.theme.fg("muted", fit(` ${item.detailPath}`, safeWidth)));
	...
	private wrapped(paragraphs: readonly string[], width: number, color: "muted" | "dim"): string[] {
		...
		return paragraphs.flatMap((paragraph) => wrapText(paragraph, inner).map((line) => this.theme.fg(color, fit(` ${line}`, width))));
	}

	private bullets(lines: readonly string[], width: number, color: "muted" | "dim"): string[] {
		const inner = Math.max(1, width - 3);
		return lines.flatMap((line) => wrapText(line, inner).map((part, index) => this.theme.fg(color, fit(`${index === 0 ? " • " : "   "}${part}`, width))));
	}
}
...
function composeRows(top: string[], bottom: string[], rowCount: number, width: number): string[] {
	...
	if (top.length + bottom.length <= rowCount) return [...top, ...Array.from({ length: rowCount - top.length - bottom.length }, () => " ".repeat(width)), ...bottom];
