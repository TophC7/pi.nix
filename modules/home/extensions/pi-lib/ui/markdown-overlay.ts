...
import { openDialog, renderDialogDivider, renderDialogFooter, renderDialogHeader, type DialogContent } from "./dialog.ts";
...
class MarkdownOverlayChrome implements DialogContent {
	...
	render(width: number): string[] {
		const safeWidth = Math.max(1, width);
		return [
			renderDialogHeader({ title: this.title, theme: this.theme, width: safeWidth }),
			renderDialogDivider({ theme: this.theme, width: safeWidth }),
			...this.body.render(safeWidth),
			renderDialogDivider({ theme: this.theme, width: safeWidth }),
			renderDialogFooter({ theme: this.theme, width: safeWidth, keys: MARKDOWN_FOOTER_KEYS }),
		];
	}
...
export function showMarkdownOverlay(
	...
	return openDialog(
		ctx,
		({ theme, close }) => new MarkdownOverlayChrome(theme, options.title, new MarkdownOverlayBody(theme, options.markdown, close, visibleRows, paddingX)),
		{ width: options.width ?? "96%", maxHeight: options.maxHeight ?? "94%", padding: 0, borderStyle: "square" },
	);
}
