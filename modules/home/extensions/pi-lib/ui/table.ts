...
export class SearchableTable<T> implements UiComponentLike {
  ...
  render(width: number): string[] {
    ...
    const visibleRows = Math.max(
      1,
      this.visibleRowsOverride ?? this.options.visibleRows ?? Math.min(8, rows.length || 1)
    )
    this.clampSelection()
    ...
    if (rows.length === 0) {
      lines.push(
        this.options.dim?.(fitLine(this.options.emptyLabel ?? 'No rows', safeWidth)) ??
          fitLine(this.options.emptyLabel ?? 'No rows', safeWidth)
      )
      return lines
    }
  ...
  private renderRow(row: T, index: number, columnWidths: readonly number[], width: number): string {
    ...
    const cells = this.options.columns.map((column, columnIndex) =>
      fitLine(column.render(row), columnWidths[columnIndex] ?? 1)
    )
