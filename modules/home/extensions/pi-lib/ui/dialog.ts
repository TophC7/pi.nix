...
import { openOverlay, type OverlayHandle, type OverlayOptions } from "./overlay.ts";
...
export function openDialog(
	...
	return openOverlay(
		ctx,
		({ tui, theme, close }) => new DialogFrame(factory({ tui, theme, close }), theme, options),
		{ width: options.width, maxHeight: options.maxHeight },
	);
}
...
export function renderDialogHeader(options: DialogHeaderOptions): string {
	...
	const searchBox = renderDialogSearchBox(options.searchInput, options.theme, options.searchBoxWidth ?? clampInt(Math.floor(safeWidth * 0.18), SEARCH_BOX_MIN_WIDTH, SEARCH_BOX_MAX_WIDTH));
...
export function renderDialogFooter(options: DialogFooterOptions): string {
	...
	const formatted = options.keys.map(({ key, label }) =>
		`${options.theme.fg("muted", options.theme.bold(key))} ${options.theme.fg("dim", label)}`,
	);
