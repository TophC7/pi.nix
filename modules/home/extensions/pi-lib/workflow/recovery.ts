...
export function recoverStaleWorkflow(
  pi: ExtensionAPI,
  ...
  try {
    ...
    const activeTools = restoreWorkflowTools(pi, {
      previousActiveTools: targetTools
    })
...
export function latestWorkflowMarker(
  ctx: ExtensionContext,
  markerType = 'pi-workflow-state'
...
function notifyRecovery(ctx: ExtensionContext, marker: WorkflowMarker, _action: WorkflowRecoveryAction): void {
  ctx.ui.notify(
    `Recovered stale ${marker.profileId} workflow with safe default tools; old active-tool snapshot was not restored.`,
    'warning'
  )
}
