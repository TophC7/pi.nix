...
export class WorkflowToolLeaseManager {
	...
	acquire(pi: ExtensionAPI, target: WorkflowToolLeaseTarget, options: WorkflowToolLeaseAcquireOptions = {}): WorkflowToolLease {
	...
	release(pi: ExtensionAPI, token: string, reason = "release"): WorkflowToolLeaseReleaseResult {
		...
		if (index < 0) return { token, status: "already_released", activeTools: pi.getActiveTools() };

		const record = this.records[index];
		if (record.releaseRequested) return { token, status: "already_released", activeTools: pi.getActiveTools() };
	...
	private drainReleasedLeases(pi: ExtensionAPI, token: string): WorkflowToolLeaseReleaseResult {
		...
		if (released.length === 0) return { token, status: "already_released", activeTools: pi.getActiveTools() };
