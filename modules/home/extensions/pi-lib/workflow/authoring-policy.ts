...
export function evaluateAuthoringToolCall<TMode extends WorkflowId>(
	...
	if (profile.mutation.fileMutationTools === "block" && (event.toolName === "write" || event.toolName === "edit")) {
		return { block: true, reason: `${event.toolName} blocked during ${mode}; use workflow save tools.` };
	}
	if (mode === "spec-authoring" && SWORM_WRITE_TOOLS.has(event.toolName ?? "") && !isSpecFinalizationActive()) {
		return { block: true, reason: blockedToolReason(mode, event.toolName ?? "tool") };
	}
	...
	if (blocked) {
		return { block: true, reason: `Mutating shell command blocked during ${mode}: ${blocked.reason}.` };
	}
}

export function commandForShellPolicy(input: unknown): string | undefined {
	const payload = input as { command?: unknown; [ORIGINAL_BASH_COMMAND_KEY]?: unknown };
	if (typeof payload[ORIGINAL_BASH_COMMAND_KEY] === "string") return unwrapFishShellCommand(payload[ORIGINAL_BASH_COMMAND_KEY]);
