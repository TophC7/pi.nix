import type { WorkflowBashRule, WorkflowId, WorkflowLeasePolicy, WorkflowMutationPolicy, WorkflowProfile, WorkflowRecoveryPolicy, WorkflowToolAccessPolicy } from "./types.ts";
...
function exclusiveToolAccess(
	...
	return {
		...access,
		onlyAllowedTools: uniqueTools(access.needsTools, access.optionalTools).filter((name) => !access.blockedTools.includes(name)),
	};
}
...
export const WORKFLOW_PROFILES = [
	...
	profile({
		...
		statusKey: "spec-workflow",
		toolAccess: {
			needsTools: uniqueTools(INSPECTION_TOOLS, SPEC_WORKFLOW_TOOLS, SPEC_FINALIZATION_CONTROL_TOOLS, SWORM_READ_STATUS_TOOLS),
			optionalTools: [],
			blockedTools: RAW_FILE_MUTATION_TOOLS,
	...
	profile({
		...
		toolAccess: additiveToolAccess(
			uniqueTools(IMPLEMENTATION_CORE_TOOLS, RAW_FILE_MUTATION_TOOLS, SWORM_TOOLS),
			["ask_user", "AskClaude", "subagent", "web_search", "code_search", "fetch_content", "get_search_content"],
		),
	...
	profile({
		...
		toolAccess: additiveToolAccess(["bash", "read", "grep", "find", "ls", "subagent", "edit", "write"], ["ask_user", "AskClaude"]),
		mutation: IMPLEMENTATION_MUTATION,
		lease: WORKFLOW_RUN_LEASE,
		recovery: STANDARD_RECOVERY,
	}),
