...
export function formatWorkflowPermissionsSnapshot(snapshot: WorkflowPermissionsSnapshot): string {
  const lines = [
    ...
    snapshot.profile
      ? `Profile: ${snapshot.profile.id} (${snapshot.profile.label}, ${snapshot.profile.phase})`
      : 'Profile: none',
    snapshot.profile
      ? `Mutation: files=${snapshot.profile.mutation.fileMutationTools}, shell=${snapshot.profile.mutation.mutatingShell}, implementation=${snapshot.profile.mutation.implementationWrites}`
      : undefined,
...
function restrictionExamples(snapshot: WorkflowPermissionsSnapshot): readonly string[] {
  ...
  if (snapshot.profile.mutation.fileMutationTools === 'block')
    examples.push('Example restriction: raw edit/write are blocked; use sanctioned save tools for authoring.')
  if (snapshot.profile.mutation.mutatingShell === 'block')
    examples.push(
      'Example restriction: mutating shell commands such as `rm foo` are blocked; read-only commands such as `git status` are allowed.'
    )
  return examples
}
...
function resetGuidance(
  status: string,
  leases: readonly WorkflowToolLease[],
  recovery: WorkflowRecoveryResult
): readonly string[] {
  const guidance: string[] = []
  if (status !== 'idle')
    guidance.push(
      'Use the owning workflow exit command if this state is expected; otherwise /reload will safe-reset stale markers on next start.'
    )
  if (leases.some((lease) => lease.status === 'restore_failed'))
    guidance.push('A lease restore failed; inspect lease restoreError and use /reload if active tools look wrong.')
  if (recovery.action === 'safe_reset')
    guidance.push('Last startup safe-reset stale workflow state and did not restore old active-tool snapshots.')
