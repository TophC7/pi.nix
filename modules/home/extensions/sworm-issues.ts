...
export function callBridge<T = unknown>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (env.protocolVersion !== '1') {
    return Promise.reject(
      new SwormBridgeError(
        'protocol_mismatch',
        `Sworm issue bridge protocol mismatch: expected 1, got ${env.protocolVersion}`
      )
    )
  }
  ...
  if (cwd !== projectPath) {
    return Promise.reject(
      new SwormBridgeError('project_mismatch', `Sworm bridge project mismatch: cwd=${cwd}; env=${projectPath}`)
    )
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      finish(() =>
        reject(new SwormBridgeError('timeout', `Sworm issue bridge timed out after ${REQUEST_TIMEOUT_MS}ms`))
      )
    }, REQUEST_TIMEOUT_MS)
...
function isEpic(value: unknown): value is EpicSummary {
  return (
    isRecord(value) &&
    typeof value.id === 'string' &&
    String(value.id).startsWith('EPIC-') &&
    typeof value.title === 'string'
  )
}
...
function isIssueDetail(value: unknown): value is {
  issue: IssueSummary
...
function formatIssueDetail(detail: {
  ...
  if (detail.dependsOn?.length)
    lines.push(`depends on: ${detail.dependsOn.map((dep) => dep.dependsOnIssueId).join(', ')}`)
...
export default function swormIssuesExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      status: Type.Optional(EpicStatus),
      priority: Priority
    }),
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      'Use sworm_issue_ready before choosing durable project work when Sworm issue memory is available.'
    ],
    parameters: Type.Object({
      epicId: Type.Optional(Type.String()),
      limit: Limit
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      'Create durable tasks, blockers, follow-ups, or discoveries that should survive this agent session.'
    ],
    parameters: Type.Object({
      title: Type.String(),
      description: MaybeString,
  ...
  pi.registerTool({
    ...
    execute: async (_id, params) =>
      toolResult('issue.claim', {
        issueId: params.issueId,
        assigneeKind: params.assigneeKind ?? 'agent',
        assigneeId: params.assigneeId ?? swormAgentId(),
        actor: swormAgentId()
      })
  })
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      'Use comments for durable findings, blockers, evidence, manual checks, and completion summaries.'
    ],
    parameters: Type.Object({
      issueId: Type.String(),
      body: Type.String(),
    ...
    execute: async (_id, params) =>
      toolResult('comment.add', {
        issueId: params.issueId,
        body: params.body,
        author: params.author ?? swormAgentId(),
        actor: swormAgentId()
      })
  })
  ...
  pi.registerTool({
    ...
    execute: async (_id, params) =>
      toolResult('comment.update', {
        commentId: params.commentId,
        input: { body: params.body, actor: swormAgentId() }
      })
  })
  ...
  pi.registerCommand('ready', {
    ...
    handler: async (args, ctx) => {
      ...
      try {
        const ready = await callBridge<IssueSummary[]>('issue.ready', {
          filters: { limit }
        })
  ...
  pi.registerCommand('claim', {
    ...
    handler: async (args, ctx) => {
      ...
      try {
        const issue = await callBridge('issue.claim', {
          issueId,
          assigneeKind: 'agent',
          assigneeId: swormAgentId(),
          actor: swormAgentId()
        })
