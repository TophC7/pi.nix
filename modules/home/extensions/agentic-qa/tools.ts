...
export function registerQaTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    promptSnippet:
      'Use qa_report for final QA reports. Every pass/fail check and bug claim needs evidenceIds referencing evidence entries. Failed reports need screenshot evidence.',
    ...
    execute: async (_id, params, _signal, _onUpdate, ctx) => {
      ...
      const artifacts = writeQaArtifacts(ctx.cwd, params, result)
      const artifactSummary = artifacts.written
        ...
        : artifacts.blocked
          ...
          : ''
      return {
        content: [{ type: 'text' as const, text: `${result.markdown}${artifactSummary}` }],
