...
const GIT_SETTINGS = [
	...
	{ value: "thinking", label: "thinking", description: "Pick and save shared /commit and /pr thinking level" },
	{ value: "status", label: "status", description: "Show shared git action config" },
	{ value: "reset", label: "reset", description: "Clear shared git action config" },
...
function getGitCompletions(prefix: string): AutocompleteItem[] | null {
	...
	const items = fuzzyFilter([...GIT_SETTINGS], trimmed, (setting) => setting.value)
		.map((setting) => ({
			value: setting.value,
			label: setting.label,
