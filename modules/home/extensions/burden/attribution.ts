...
function parseAgents(contextBlock: string): readonly BurdenEntry[] {
	...
	return matches.map((match, index) => {
		...
		return entry(`agents:${path}`, path, "agents", content, { source: { kind: "file", path } });
	});
}
...
function parseSkills(skillsBlock: string): readonly BurdenEntry[] {
	...
	return [...skillsBlock.matchAll(skillPattern)].map((match, index) => {
		...
		return entry(`skill:${name}`, name, "skills", full, {
			...
			children: description ? [syntheticEntry(`skill:${name}:description`, "description", "skills", estimateTokens(description), description.length)] : undefined,
		});
	});
}
...
export function buildBurdenReport(pi: ExtensionAPI, ctx: ExtensionContext): BurdenReport {
	...
	if (baseEnd > 0) {
		sections.push(entry("base", "Base prompt", "base-prompt", prompt.slice(0, baseEnd), { source: { kind: "prompt", name: "base" } }));
	...
	if (baseEnd >= 0 && nextMainSection > baseEnd) {
		...
		if (content) sections.push(entry("custom-system", "SYSTEM.md / APPEND_SYSTEM.md", "custom-system-prompt", content, { source: { kind: "prompt", name: "custom-system" } }));
	...
	if (projectContextIdx >= 0) {
		...
		sections.push(entry("agents", "AGENTS.md files", "agents", content, { children: parseAgents(content), source: { kind: "prompt", name: "project-context" } }));
		ranges.push({ start, end: actualEnd });
	}

	if (skillsPreambleIdx >= 0) {
		const start = skillsPreambleIdx + 2;
		const end = availableSkillsEnd >= 0 ? availableSkillsEnd + "</available_skills>".length : (metadataIdx >= 0 ? metadataIdx : prompt.length);
		const content = prompt.slice(start, end);
		const xml = availableSkillsStart >= 0 && availableSkillsEnd >= 0
			? prompt.slice(availableSkillsStart, availableSkillsEnd + "</available_skills>".length)
			: "";
		sections.push(entry("skills", "Skills", "skills", content, { children: parseSkills(xml), source: { kind: "prompt", name: "available-skills" } }));
		ranges.push({ start, end });
	}

	if (metadataIdx >= 0) {
		const start = metadataIdx + 1;
		sections.push(entry("metadata", "Metadata", "metadata", prompt.slice(start), { source: { kind: "prompt", name: "metadata" } }));
	...
	gaps.push({
		...
		reason: "Public Pi APIs expose the assembled system prompt, tools, active tools, skills in prompt XML, and context usage, but do not expose every extension contribution as a separate source segment.",
		tokens: 0,
		chars: 0,
	});
	...
	const unknown = syntheticEntry("unknown", "Unknown", "unknown", unknownTokens, unknownChars, {
		source: { kind: "unknown" },
		children: unknownTexts.map((text, index) => entry(`unknown:${index}`, `Unattributed segment ${index + 1}`, "unknown", text, { source: { kind: "unknown" } })),
	});
