...
export class BurdenExplorer implements DialogContent {
	...
	private tableBody(width: number): string[] {
		...
		const selectedIndex = Math.max(0, visibleRows.findIndex((row) => row.id === this.selectedId));
		...
		if (panes.stacked) {
			...
			lines.push(renderDialogDivider({ theme: this.options.theme, width, splitAt: panes.tree }));
	...
	private renderTreeLines(rows: readonly BurdenRowView[], meta: ReadonlyMap<string, TreeRowMeta>, paneWidth: number, rowCount: number, isSearching: boolean): string[] {
		if (rows.length === 0) {
		...
		const lines = window.map((row) => isSearching
	...
	private renderDetailLines(row: BurdenRowView | undefined, paneWidth: number, rowCount: number): string[] {
		...
		if (!row) {
			return padRows([
				blank,
				`  ${theme.fg("dim", "Select a row to see details.")}`,
			], rowCount, safeWidth);
		}
		...
		if (breadcrumb) lines.push(this.padPane(`  ${theme.fg("dim", truncateToWidth(breadcrumb, contentWidth, "…", true))}`, safeWidth));
		lines.push(blank);
		lines.push(this.padPane(`  ${theme.fg("muted", `${formatTokens(row.tokens)} tokens`)}`, safeWidth));
		lines.push(this.padPane(`  ${theme.fg("dim", `${percent(pctUsed)} of used${pctCtx !== undefined ? ` · ${percent(pctCtx)} of context` : ""}`)}`, safeWidth));
		...
		if (source) {
			...
			if (source.name) lines.push(this.padPane(`  ${theme.fg("dim", truncateToWidth(source.name, contentWidth, "…", true))}`, safeWidth));
			if (source.path) lines.push(this.padPane(`  ${theme.fg("dim", truncateToWidth(source.path, contentWidth, "…", true))}`, safeWidth));
			lines.push(blank);
		}

		if (row.hasChildren) {
			lines.push(this.padPane(`  ${theme.fg("muted", `${row.childIds.length} child${row.childIds.length === 1 ? "" : "ren"}`)}`, safeWidth));
			lines.push(this.padPane(`  ${theme.fg("dim", this.expanded.has(row.id) ? "→ collapse to hide" : "→ expand to inspect")}`, safeWidth));
			lines.push(blank);
		}
	...
	private previewSelected(): void {
		...
		if (preview) {
			this.preview = {
				row,
				body: new MarkdownOverlayBody(this.options.theme, preview.markdown, () => {
					this.preview = undefined;
					this.status = `Back from preview: ${row.label}`;
				}, this.previewRowBudget(), 1),
			};
			return;
		}
	...
	private previewForRow(row: BurdenRowView): { title: string; markdown: string } | undefined {
		...
		try {
			...
			return { title: sourcePath, markdown: sourceMarkdown(row, sourcePath, content) };
		} catch (error) {
...
function computeTreeMeta(rows: readonly BurdenRowView[], view: BurdenViewModel): Map<string, TreeRowMeta> {
	...
	for (const row of rows) {
		...
		result.set(row.id, { isLast: isLastMap.get(row.id) ?? true, ancestorMask: mask });
	}
	return result;
}
