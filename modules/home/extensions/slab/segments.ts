...
function statusText(ctx: SlabSegmentRenderContext): string[] {
	...
	return ctx.state.statuses
		.map((entry) => {
			const raw = typeof entry.text === "function" ? entry.text({ width, capabilities: ctx.render, now: ctx.render.now, tick: ctx.render.tick }) : entry.text;
...
export function renderSegment(ctx: SlabSegmentRenderContext, segment: SlabSegmentDefinition): SlabSegmentRenderResult | undefined {
