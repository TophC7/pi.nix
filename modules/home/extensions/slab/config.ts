...
export function cloneSlabConfig(config: SlabConfig): SlabConfig {
  return {
    ...
    model: {
      customNames: { ...config.model.customNames },
      showThinking: config.model.showThinking
    },
    git: { ...config.git },
  ...
  return {
    ...
    model: {
      customNames: Object.fromEntries(
        Object.entries(customNames).filter((entry): entry is [string, string] => typeof entry[1] === 'string')
      ),
