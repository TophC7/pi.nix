...
function sessionEntries(ctx: ExtensionContext): readonly unknown[] {
	const manager = ctx.sessionManager as unknown as { getEntries?: () => readonly unknown[] };
...
export function computeUsageTotals(ctx: ExtensionContext): SlabUsageTotals {
	...
	const usage: SlabUsageTotals = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0 };
...
export function createSlabRuntimeState(ctx: ExtensionContext, snapshot: UiSnapshot, clock: UiRenderClock, config: SlabConfig, options: CreateSlabRuntimeStateOptions = {}): SlabRuntimeState {
	const cwd = ctx.cwd;
