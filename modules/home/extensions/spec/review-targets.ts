...
export type ReviewTargetParseResult = { ok: true; target: ReviewTarget } | { ok: false; error: string }

export const REVIEW_TARGET_USAGE =
  'working-tree | staged | range <base>..<head> | branch <name> [base] | paths <path...> | paste [text] | freeform <path>'
...
function parseBranch(rest: string[]): ReviewTargetParseResult {
  ...
  return {
    ok: true,
    target: base ? { kind: 'branch', name, base } : { kind: 'branch', name }
  }
}
