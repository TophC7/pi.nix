...
async function snapshotTrackedTree(pi: ExtensionAPI, ctx: ExtensionCommandContext): Promise<TreeSnapshot> {
  const status = await pi.exec('git', ['status', '--porcelain=v1', '-uall'], {
    cwd: ctx.cwd,
    signal: ctx.signal
  })
  const statusLines = (status.stdout ?? '').split('\n').filter(Boolean).sort()
  const trackedList = await pi.exec('git', ['ls-files', '-z'], {
    cwd: ctx.cwd,
    signal: ctx.signal
  })
...
export async function runPlan(pi: ExtensionAPI, ctx: ExtensionCommandContext, args?: string): Promise<void> {
  ...
  try {
    const scoutResponse = await runSubagent(
      pi,
      ctx,
      {
        tasks: [
          ...
          {
            agent: 'spec.plan-risk-scout',
            task: planRiskScoutTask(description)
          }
        ],
        context: 'fresh',
        agentScope: 'both'
    ...
    ctx.ui.notify(
      'Plan discovery complete. Manual handoff prepared for parent agent interview, synthesis, hardening, and save.',
      'info'
    )
    await handoff({
      pi,
...
export async function runSpecNew(pi: ExtensionAPI, ctx: ExtensionCommandContext, args?: string): Promise<void> {
  ...
  if (!planPath) {
    const choice = await ctx.ui.select('/spec:new needs a plan draft', [
  ...
  try {
    const verifierResponse = await runSubagent(
      pi,
      ctx,
      {
    ...
    ctx.ui.notify(
      'Spec verification complete. Manual handoff prepared for parent agent interview, draft, review, hardening, and save.',
      'info'
    )
    await handoff({
      pi,
...
export async function runSpecSync(_pi: ExtensionAPI, ctx: ExtensionCommandContext, args?: string): Promise<void> {
  ...
  try {
    ...
    const changedOutside =
      before.replace(/<!-- spec-sync:start -->[\s\S]*?<!-- spec-sync:end -->/, '') !==
      after.replace(/<!-- spec-sync:start -->[\s\S]*?<!-- spec-sync:end -->/, '')
    ctx.ui.notify(
      `Synced ${spec.indexPath}${changedOutside ? ' (warning: outside marker changed)' : ''}`,
      changedOutside ? 'error' : 'info'
    )
  } catch (error) {
    ctx.ui.notify(formatBridgeError(error), 'error')
  }
...
export async function runSpecCheck(pi: ExtensionAPI, ctx: ExtensionCommandContext, args?: string): Promise<void> {
  ...
  for (const command of [...runChecks, ...invariantChecks]) {
    const result = await pi.exec('fish', ['-lc', command], {
      cwd: ctx.cwd,
      signal: ctx.signal,
      timeout: 120000
    })
...
export async function runSpecStatus(pi: ExtensionAPI, ctx: ExtensionCommandContext, args?: string): Promise<void> {
  ...
  const askClaude = pi
    .getAllTools()
    .some((tool) => tool.name === 'AskClaude' || /claude/i.test(`${tool.name} ${tool.description ?? ''}`))
