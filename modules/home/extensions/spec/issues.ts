...
export async function loadSpecSwormState(spec: SpecInfo): Promise<SpecSwormState> {
	...
	const [epic, issues, ready] = await Promise.all([
		...
		callBridge<SwormIssue[]>("issue.list", { filters: { epicId, includeArchived: true, limit: 500 } }),
		callBridge<SwormIssue[]>("issue.ready", { filters: { epicId, limit: 50 } }),
	]);
	const comments = (await Promise.all(issues.map((issue) => callBridge<SwormComment[]>("comment.list", { issueId: issue.id }).catch(() => [])))).flat();
	return { epicId, epic, issues, ready, comments, summary: summarizeIssues(issues, ready, comments) };
}
...
export function summarizeIssues(issues: SwormIssue[], ready: SwormIssue[], comments: SwormComment[]): TaskSummary {
	...
	return { counts, readyLine, blockers: [...new Set(blockers)], manualChecks: [...new Set(manualChecks)] };
}
...
export function formatCounts(counts: Record<string, number>): string {
	...
	const extra = Object.keys(counts).filter((key) => !order.includes(key)).sort().map((key) => `${key}:${counts[key]}`);
