...
export function validateReviewPlanDraft(
  content: string,
  options: ReviewPlanValidationOptions = {}
...
function validateSections(content: string, errors: string[]): void {
  for (const section of REQUIRED_SECTIONS) {
    if (!new RegExp(`^## ${escapeRegExp(section)}\\s*$`, 'm').test(content))
      errors.push(`Missing required review-plan section: ${section}`)
  }
}
