...
function missingRequiredFields(
  fields: Partial<Record<'problem' | 'evidence' | 'fixDirection' | 'specPromotionNote', string>>
): string[] {
...
function renderReport(
  ...
  const verdict = findings.some((finding) => finding.severity === 'Blocking' || finding.severity === 'Required')
    ? 'Request Changes'
    : 'Approve'
  return [
    '# /plan:review report',
    ...
    quarantined.length
      ? `- ${quarantined.length} malformed card(s) quarantined; rerun the affected agents or repair the schema before promotion.`
      : '',
    '',
    '## Verdict',
    verdict,
...
function renderQuarantined(quarantined: QuarantinedReviewCard[]): string {
  return quarantined
    .map((card, index) => `- #${index + 1} ${card.rawHeader || '<missing header>'} — ${card.reasons.join('; ')}`)
    .join('\n')
}
...
function renderPlanDraft(findings: SynthesizedReviewFinding[], target: ReviewTarget): string {
  ...
  return [
    ...
    required.length
      ? required
          .map(
            (finding) => `- ${finding.severity}: ${finding.location} — ${finding.problem} Fix: ${finding.fixDirection}`
          )
          .join('\n')
    ...
    suggestions.length
      ? suggestions
          .map((finding) => `- ${finding.location} — ${finding.problem} Fix: ${finding.fixDirection}`)
          .join('\n')
      : 'No Suggestions.',
    '',
    '## Promotion notes',
