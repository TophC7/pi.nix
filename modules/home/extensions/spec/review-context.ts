  ...
  switch (target.kind) {
    ...
    case 'branch':
      await captureRange(
        ...
        target.base ?? 'HEAD',
        target.name,
        target.base ? 'branch range' : 'branch vs HEAD'
      )
      break
    case 'paths':
...
async function captureWorkingTree(
  ...
  await captureNamedDiff(
    pi,
    ctx,
    builder,
    ['diff', '--cached', '--find-renames'],
...
async function captureRange(
  ...
  await captureNamedDiff(
    ...
    ctx,
    builder,
    ['diff', '--find-renames', base, head],
    ['diff', '--name-status', '-z', base, head],
    `${label}: ${base}..${head}`
  )
}

async function captureNamedDiff(
  pi: ExtensionAPI,
  ctx: ExtensionCommandContext,
  builder: ContextBuilder,
  diffArgs: string[],
  ...
  for (const entry of entries) {
    ...
    const diff = await execGitTextBounded(
      ctx,
      [...diffArgs, '--', entry.path],
      Math.min(REVIEW_CONTEXT_PER_FILE_LIMIT, builder.remainingBytes())
    )
...
function capturePath(ctx: ExtensionCommandContext, builder: ContextBuilder, path: string, title: string): void {
  ...
  if (!stat.isFile()) {
    builder.add(
      `${title}: ${path}`,
      `Not a regular file (${stat.isDirectory() ? 'directory' : 'special file'}); content omitted.`,
      { fileScoped: true }
    )
    return
  }
  ...
  if (isBinary(data.content)) {
    builder.add(
      `${title}: ${path}`,
      `Binary file summarized only (${data.totalBytes} bytes, sampled ${data.bytesRead} bytes).`,
      { fileScoped: true }
    )
    return
  }
  ...
  if (data.truncated) {
    ...
    builder.add(`${title}: ${path}`, `${takeUtf8(text, bodyLimit)}${note}`, {
      fileScoped: true
    })
...
async function execGitTextBounded(
  ctx: ExtensionCommandContext,
  args: string[],
  maxBytes: number
  ...
  if ((result.code ?? 1) !== 0 && !result.truncated)
    throw new Error(`git ${args.join(' ')} failed: ${result.stderr || result.stdout}`.trim())
  ...
  return {
    text: `${takeUtf8(result.stdout, bodyLimit)}${note}`,
    note: `git ${args.join(' ')} truncated after ${result.stdoutBytes} stdout bytes.`
  }
}
