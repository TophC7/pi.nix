...
export function registerSpecWorkflowTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      content: Type.String({
        description: 'Complete markdown plan with required frontmatter'
      })
    }),
    async execute(_toolCallId, params) {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Saved ${result.path} (${result.bytes} bytes).`
          }
        ],
        details: result
      }
  ...
  pi.registerTool({
    ...
    description:
      'Hand off a saved .sworm/plans/<file>.md to /spec:new. Exits plan-authoring mode and triggers spec-authoring (which is where Sworm issue creation runs). Use only after save_plan_draft has succeeded and the user has chosen to promote.',
    parameters: Type.Object({
      path: Type.String({
        description: 'Path returned by save_plan_draft, e.g. .sworm/plans/2026-05-07-my-plan.md'
      })
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (!(await requireSwormBridge(ctx))) {
        ...
        return {
          ...
          details: { path: params.path, error: 'sworm_bridge_unavailable' },
          isError: true
        }
      }
      exitMode(pi, ctx)
      ...
      return {
        ...
        details: { path: params.path, outcome }
      }
    }
  })
  ...
  pi.registerTool({
    ...
    description:
      'Open the finalization gate for /spec:new after explicit final user approval. Enables only sanctioned save tools and Sworm mutator/config tools until agent_end.',
    parameters: Type.Object({
      approval: Type.String({
        description:
          "Short record of the user's explicit final approval, e.g. 'User approved final spec and Sworm writes.'"
      })
    }),
    async execute(_toolCallId, params) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      content: Type.String({
        description: 'Complete markdown spec file with required frontmatter'
      })
    }),
    async execute(_toolCallId, params) {
