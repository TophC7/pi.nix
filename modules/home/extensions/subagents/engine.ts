...
interface PiRuntimeModule {
	...
	ModelRegistry: { create(authStorage: unknown): { find(provider: string, modelId: string): unknown } };
...
function asPiRuntime(value: unknown): PiRuntimeModule {
	if (!value || typeof value !== "object") throw new Error("@mariozechner/pi-coding-agent did not export a module object.");
	const requiredMembers = ["AuthStorage", "ModelRegistry", "DefaultResourceLoader", "SessionManager", "createAgentSession"] as const;
...
export async function runSubagents(request: SubagentRunRequest, options: RunSubagentsOptions): Promise<SubagentRunResult> {
	...
	request.parentSignal?.addEventListener("abort", cancelChildren, { once: true });

	try {
		const results = request.mode === "parallel"
			? await Promise.all(request.slots.map((slot, index) => runSlot(slot, index, request, agentMap, agentDir, childControllers, state, options)))
			: request.mode === "chain"
...
async function runChainSlotsGrouped(
	...
	for (let index = 0; index < request.slots.length;) {
		...
		const groupResults = await Promise.all(group.map((item) => runSlot(item.slot, item.index, request, agents, agentDir, childControllers, state, options)));
...
async function runSlot(
	...
	request.parentSignal?.addEventListener("abort", onParentAbort, { once: true });
	...
	try {
		...
		const effectiveAgent = { ...agent, model: plan.model ?? agent.model, thinking: plan.thinking ?? agent.thinking };
		session = await createRunnerSession(effectiveAgent, request.cwd, agentDir, controller.signal, options.sessionFactory);
...
async function createRunnerSession(
	...
	const loader = new pi.DefaultResourceLoader({
		cwd,
		agentDir,
		noExtensions: tools !== undefined && tools.every((tool) => BUILTIN_TOOLS.includes(tool as (typeof BUILTIN_TOOLS)[number])),
		...
		appendSystemPromptOverride: (base: unknown[]) => agent.systemPromptMode === "append" ? [...base, agent.systemPrompt] : base,
	});
	await loader.reload();
...
function validateActiveTools(agent: DiscoveredAgent, session: RunnerSession): void {
	...
	if (missing.length > 0 || extra.length > 0) {
		throw new Error(`Active tool mismatch for ${agent.runtimeName}: missing [${missing.join(", ")}], extra [${extra.join(", ")}]`);
	}
}

function handleSessionEvent(event: SessionEvent, slot: SubagentSlotResult, state: SubagentRunState, options: RunSubagentsOptions): void {
	if (event.type === "tool_execution_start") {
		...
		emitEvent(state, options, { timestamp: Date.now(), slotId: slot.id, type: "tool-start", tool: slot.tools.at(-1) });
		...
		emitEvent(state, options, { timestamp: Date.now(), slotId: slot.id, type: "tool-end", tool });
		...
		if (delta) {
			...
			emitEvent(state, options, { timestamp: Date.now(), slotId: slot.id, type: "slot-output", text: delta });
		}
...
function failSlot(slot: SubagentSlotResult, message: string, state: SubagentRunState, options: RunSubagentsOptions): SubagentSlotResult {
	slot.status = "failed";
	slot.error = message;
...
function cancelSlot(slot: SubagentSlotResult, state: SubagentRunState, options: RunSubagentsOptions): SubagentSlotResult {
	slot.status = "cancelled";
	slot.cancelled = true;
...
function finishSlot(slot: SubagentSlotResult, state: SubagentRunState, options: RunSubagentsOptions, type: "slot-end" | "error" | "cancelled" = "slot-end", error?: string): void {
