...
import { parseSystemPromptMode, parseThinking, type SubagentSystemPromptMode, type SubagentThinkingLevel } from "./request.ts";
...
function buildAgentRoots(options: DiscoverAgentsOptions, agentDir: string, scope: AgentScope): AgentRoot[] {
	...
	if (scope === "project" || scope === "both") {
		roots.push({ dir: options.projectAgentsDir ?? join(options.cwd, ".pi", "agents"), source: "project" });
	}
	return roots;
}
