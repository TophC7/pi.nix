...
const ReasoningClaim = Type.Object({
  external_id: Type.String({
    description: 'Stable local id for this observe call, e.g. c1.'
  }),
...
const ReasoningEdge = Type.Object({
  from: Type.String({
    description: 'External id from this call or prior claim UUID prefix.'
  }),
  to: Type.String({
    description: 'External id from this call or prior claim UUID prefix.'
  }),
...
export function registerBuddyTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    description:
      'Store a durable Buddy memory when the user explicitly asks to remember something or states a stable preference worth preserving.',
  ...
  pi.registerTool({
    ...
    label: 'Buddy Observe',
    description:
      'Record a brief observation after meaningful work and return Buddy reaction plus XP. In guard mode, include only directly evidenced claims/edges; never fabricate claims.',
    parameters: Type.Object({
      ...
      cwd: Type.Optional(
        Type.String({
          description: 'Workspace path hint for guard-mode session scoping.'
        })
      )
    }),
    renderShell: 'self',
