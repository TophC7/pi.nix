...
const BUDDY_ACTIONS = [
  {
    value: 'status',
    label: 'status',
    description: 'Show current companion card.'
  },
  {
    value: 'hatch',
    label: 'hatch',
    description: 'Explicitly hatch a new companion.'
  },
  { value: 'pet', label: 'pet', description: 'Pet Buddy and award small XP.' },
  {
    value: 'mode',
    label: 'mode',
    description: 'Set voice mode: backseat, skillcoach, both.'
  },
  {
    value: 'guard',
    label: 'guard',
    description: 'Toggle guard-mode reasoning observations.'
  },
  {
    value: 'remember',
    label: 'remember',
    description: 'Store a Buddy memory.'
  },
  {
    value: 'forget',
    label: 'forget',
    description: 'Forget memories, progress, or all state.'
  },
  {
    value: 'reasoning',
    label: 'reasoning',
    description: 'Show or purge guard-mode reasoning state.'
  },
  ...
  { value: 'unmute', label: 'unmute', description: 'Restore Buddy reactions.' },
  {
    value: 'respawn',
    label: 'respawn',
    description: 'Release current companion.'
  }
] as const
      ...
      [
        {
          value: 'backseat',
          label: 'backseat',
          description: 'Small nudges and reactions.'
        },
        {
          value: 'skillcoach',
          label: 'skillcoach',
          description: 'More explicit coaching voice.'
        },
        {
          value: 'both',
          label: 'both',
          description: 'Backseat reactions plus coaching.'
        },
        {
          value: 'guard=on',
          label: 'guard=on',
          description: 'Enable reasoning graph guard.'
        },
        {
          value: 'guard=off',
          label: 'guard=off',
          description: 'Disable reasoning graph guard.'
        }
      ],
      lastToken(prefix)
    )
      ...
      [
        {
          value: 'memories',
          label: 'memories',
          description: 'Forget stored memories only.'
        },
        {
          value: 'progress',
          label: 'progress',
          description: 'Reset XP/progress only.'
        },
      ...
      [
        {
          value: 'status',
          label: 'status',
          description: 'Show current guard graph counts.'
        },
        {
          value: 'purge',
          label: 'purge',
          description: 'Purge reasoning session/all graph state.'
        }
      ],
      lastToken(prefix)
    )
...
function summarizeCommandResult(action: string, text: string, details: unknown): string {
  ...
  if (action === 'hatch' && companion?.name && companion.species) {
    ...
    return animation.length <= 700
      ? animation
      : `Hatched ${companion.name} the ${companion.species}. Run /buddy for the full Buddy view.`
  }
