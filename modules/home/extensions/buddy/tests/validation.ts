...
check('UI placement static gates', () => {
  ...
  const commandActions = readFileSync(join(root, 'modules/home/extensions/commands/actions.ts'), 'utf8')
  const commandIndex = readFileSync(join(root, 'modules/home/extensions/commands/index.ts'), 'utf8')
  const commandDialog = readFileSync(join(root, 'modules/home/extensions/commands/dialog.ts'), 'utf8')
  const commandPatch = readFileSync(join(root, 'modules/home/patches/pi-command-models-compact.patch'), 'utf8')
  ...
  assert(commandActions.includes("if (outcome.kind === 'queued_unverified') return") && commandActions.includes("ctx.ui.notify(`/${restore.command} config restored`"), 'command model override should survive queued handoff until agent_end restore')
  assert(commandIndex.includes("registerCommand('config'") && !commandIndex.includes("registerCommand('git'"), 'commands extension should expose /config, not /git')
  assert(commandDialog.includes('class CommandsConfigDialog') && commandDialog.includes('class ModelSearchPicker') && !commandDialog.includes('showSettingsPane'), 'commands config should use dedicated dialog with searchable model picker')
  assert(commandPatch.includes('applyCommandModelConfig(this, "compact")') && commandPatch.includes('restoreCommandModelConfig(this, commandModelRestore)'), '/compact should apply configured model/thinking and restore after compaction')
