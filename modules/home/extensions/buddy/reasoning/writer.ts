...
export function writeClaims(db: Database, sessionId: string, claims: unknown, edges: unknown): WriteResult {
  ...
  for (const claim of claimInputs) {
    ...
    db.query(
      'INSERT INTO reasoning_claims (id, session_id, speaker, text, basis, confidence, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ...
  for (const edge of edgeInputs) {
    ...
    db.query(
      'INSERT INTO reasoning_edges (id, session_id, from_claim, to_claim, type, created_at) VALUES (?, ?, ?, ?, ?, ?)'
...
export function loadRecentClaims(
  db: Database,
  sessionId: string,
  limit = REASONING_CONFIG.RECENT_CLAIMS_CONTEXT
): StoredClaim[] {
  return db
    .query('SELECT * FROM reasoning_claims WHERE session_id = ? ORDER BY created_at DESC LIMIT ?')
...
function resolveEndpoint(
  ...
  const row = db
    .query('SELECT id FROM reasoning_claims WHERE session_id = ? AND (id = ? OR substr(id, 1, 8) = ?) LIMIT 1')
    .get(sessionId, ref, ref.toLowerCase()) as { id: string } | null
  return row?.id ?? null
}
...
function sanitizeClaim(text: string): string {
  return text
    ...
    .replace(/[{}$]/g, '')
    .trim()
    .slice(0, REASONING_CONFIG.MAX_CLAIM_TEXT_LENGTH)
}
...
function pruneClaimOverflow(db: Database, sessionId: string): void {
  ...
  `).run(
    sessionId,
    sessionId,
    REASONING_CONFIG.MAX_CLAIMS_PER_SESSION,
    sessionId,
    REASONING_CONFIG.MAX_CLAIMS_PER_SESSION
  )
  db.query(`DELETE FROM reasoning_claims WHERE id IN (${staleClaims})`).run(
    sessionId,
    REASONING_CONFIG.MAX_CLAIMS_PER_SESSION
  )
}
